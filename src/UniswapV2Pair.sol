...
import './IUniversalFeeRouter.sol';
...
contract UniswapV2Pair {
    ...
    event FeeRouted(address indexed token, uint256 amount, address indexed router);
    // =======================
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            liquidity = _min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1);
        }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        // === UFR FEE EXTRACTION & ROUTING ===
        if (feeRouter != address(0)) {
            uint256 fee0 = (amount0In * FEE_BPS) / 10000;
            uint256 fee1 = (amount1In * FEE_BPS) / 10000;

            if (fee0 > 0) {
                _safeTransfer(token0, feeRouter, fee0);
                IUniversalFeeRouter(feeRouter).routeERC20(token0, fee0);
                emit FeeRouted(token0, fee0, feeRouter);
            }
            if (fee1 > 0) {
                _safeTransfer(token1, feeRouter, fee1);
                IUniversalFeeRouter(feeRouter).routeERC20(token1, fee1);
                emit FeeRouted(token1, fee1, feeRouter);
            }
        }
        // =====================================
